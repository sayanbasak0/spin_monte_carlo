...
double generate_gaussian(int J_h, int j_L_S) // Marsaglia polar method
{
    double X, r;
    double sigma = 1;
    if (J_h == 0)
...
int initialize_h_random_gaussian()
{
    ...
    for(j_S=0; j_S<dim_S; j_S=j_S+1)
    {
        ...
        for(i=0; i<no_of_sites; i=i+1)
        {
            ...
            h_random[dim_S*r_i + j_S] = sigma_h[j_S] * generate_gaussian(1, j_S);
...
int initialize_J_random_gaussian()
{
    ...
    for(j_L=0; j_L<dim_L; j_L=j_L+1)
    {
        ...
        for(i=0; i<no_of_sites; i=i+1)
        {
            ...
            J_random[2*dim_L*r_i + 2*j_L] = sigma_J[j_L] * generate_gaussian(0, j_L);
