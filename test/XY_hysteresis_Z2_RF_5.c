...
//====================      NN-interaction (J)               ====================//
    double J[2*dim_L] = { 1.0, 1.0, 1.0, 1.0 }; 
    ...
    int checkerboard_Metropolis_sweep(int iter)
    {
        ...
        while(iter > 0)
        {
            ...
            for (i=0; i < no_of_black_white_sites[black_or_white]; i++)
            {
                ...
                // if (r < update_prob)
                // {
                // double update_r = (r < update_prob) ? 1.0 : 0.0 ;
                // printf("update_r = %lf", update_r);
                    for (j_S=0; j_S<dim_S; j_S++)
                    {
                        spin[dim_S*site_index + j_S] = (r < update_prob) ? spin[dim_S*site_index + j_S] : spin_local[j_S];
                    }
...
//====================      Main                             ====================//
    ...
    int main()
    {
        ...
        load_J_config();
        printf("J loaded. \n");
        spin = calloc( dim_S*no_of_sites, sizeof(double) );
