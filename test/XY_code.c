...
double some_rand_func_0_1;
int some_rand_func_int;
...
//===============================================================================//
//====================      Lattice size                     ====================//
    int lattice_size[dim_L] = { 4, 4 }; // lattice_size[dim_L]
    int no_of_sites;
    int no_of_black_sites;
    int no_of_white_sites;
    int no_of_black_white_sites[2];

//====================      Checkerboard variables           ====================//
    // int *black_white_checkerboard[2];
    int *black_white_checkerboard;
    int *black_checkerboard;
    int *white_checkerboard;
...
//====================      MC-update iterations             ====================//
    int thermal_i = 1*10*1*1; // *=lattice_size
    int average_j = 1*10*1; // *=lattice_size
    int sampling_inter = 1; // =sampling_inter-rand()%sampling_inter

//====================      Hysteresis T!=0                  ====================//
    int hysteresis_MCS = 1; 
    int hysteresis_MCS_min = 1; 
    int hysteresis_MCS_max = 100;
    int hysteresis_repeat = 1;
    int hysteresis_MCS_multiplier = 10;

//====================      declare cuda variables           ====================//
    double *spin_device;
    double *h_random_device;
    double *J_random_device;
    int *N_N_I_device;
    int *black_white_checkerboard_device;
    double *order_device;
    double *h_device;
    double *J_device;
    double *some_rand_func_0_1_device;
    int *some_rand_func_int_device;
    double *T_device;
    int *no_of_sites_device;
    int *no_of_black_white_sites_device;
...
//====================      Custom                           ====================//
    ...
    int custom_int_pow(int base, int power)
    {
        if (power > 0)
        {
    ...
    int nearest_neighbor(int xyzi, int j_L, int k_L)
    {
        int xyzinn;
        int jj_L;
        // int temp_1 = pow(lattice_size, j);
        int temp_1 = 1;
        for(jj_L=0; jj_L<j_L; jj_L++)
        {
            temp_1 = temp_1 * lattice_size[jj_L];
        }
        // int temp_1 = custom_int_pow(lattice_size, j);
        int temp_2 = ((int)(xyzi/temp_1))%lattice_size[j_L];
        xyzinn = xyzi - (temp_2) * temp_1;
        xyzinn = xyzinn + ((temp_2 + ((k_L*2)-1) + lattice_size[j_L])%lattice_size[j_L]) * temp_1;
        return xyzinn;
    }

    int direction_index(int xyzi)
    {
        int j_L;
        // int temp_1 = pow(lattice_size, j);
        int temp_1 = 1;
        ...
        {
            site_to_dir_index[j_L] = ((int)(xyzi/temp_1))%lattice_size[j_L];
...
//====================      Initialization                   ====================//
    ...
    int initialize_nearest_neighbor_index()
    {
        ...
        N_N_I = (int*)malloc(2*dim_L*no_of_sites*sizeof(int));    
    ...
    int initialize_checkerboard_sites()
    {
        ...
        black_white_checkerboard = (int*)malloc(2*no_of_black_white_sites[0]*sizeof(int));
        // black_white_checkerboard[0] = (int*)malloc(no_of_black_white_sites[0]*sizeof(int));
        // black_white_checkerboard[1] = (int*)malloc(no_of_black_white_sites[1]*sizeof(int));
        
        // no_of_black_white_sites[0] = no_of_black_sites;
        // no_of_black_white_sites[1] = no_of_white_sites;
        
        // black_checkerboard = (int*)malloc(no_of_black_sites*sizeof(int));
        // white_checkerboard = (int*)malloc(no_of_white_sites*sizeof(int));
        ...
        for (i=0; i<no_of_sites; i++)
        {
            ...
            {
                black_white_checkerboard[0*no_of_black_white_sites[0] + black_white_index[0]] = i;
            ...
            {
                black_white_checkerboard[1*no_of_black_white_sites[0] + black_white_index[1]] = i;
        ...
        for(i=0; i<no_of_sites; i=i+1)
        {
            ...
            do
            {
                ...
                for(j_S=0; j_S<dim_S; j_S=j_S+1)
                {
                    ...
                    spin[dim_S*i+j_S] = (-1.0 + 2.0 * some_rand_func_0_1);
...
//====================      Magnetisation                    ====================//
    ...
    #ifdef iscuda
    __global__ void ensemble_device()
    {
        int tid = threadIdx.x;
        double m_j_S = 0.0;
        // if (tid < no_of_sites)
...
//====================      MonteCarlo-tools                 ====================//
    ...
    int update_spin_single(int xyzi, double* __restrict__ spin_local)
    {
        int j_S;
    ...
    // #pragma acc routine seq
    double Energy_minimum(int xyzi, double* __restrict__ spin_local, double* __restrict__ field_local)
    {
        int j_S, j_L, k_L;
    ...
    // #pragma acc routine seq
    double Energy_old(int xyzi, double* __restrict__ spin_local, double* __restrict__ field_local)
    {
        int j_S, j_L, k_L;
        ...
        {
            field_local[j_S] = 0.0;
            // #pragma omp simd reduction(+:field_local[:j_S])
            for (j_L=0; j_L<dim_L; j_L++)
            {
                for (k_L=0; k_L<2; k_L++)
                {
    ...
    // #pragma acc routine seq
    double Energy_new(int xyzi, double* __restrict__ spin_local, double* __restrict__ field_local)
    {
        int j_S;
        ...
        do
        {
            ...
            for(j_S=0; j_S<dim_S; j_S=j_S+1)
            {
                ...
                spin_local[j_S] = (1.0 - 2.0 * some_rand_func_0_1);
...
//====================      Metropolis                       ====================//
    ...
    // #pragma acc routine seq
    double update_probability_Metropolis(int xyzi)
    {
        int j_S, j_L, k_L;
        double update_prob;
        ...
        double E_old = 0; // Energy_old(xyzi, spin_local, field_local);
        for (j_S=0; j_S<dim_S; j_S++)
        {
            field_local[j_S] = 0.0;
            for (j_L=0; j_L<dim_L; j_L++)
            ...
            // field_site[dim_S*xyzi + j_S] = field_local[j_S];
            // spin_old[dim_S*xyzi + j_S] = spin[dim_S*xyzi + j_S];
            E_old += field_local[j_S] * spin[dim_S*xyzi + j_S];
        }
        double E_new = 0; // Energy_new(xyzi, spin_local, field_local);
        double s_mod = 0;
        double limit = 0.01 * dim_S;
        ...
        for(j_S=0; j_S<dim_S; j_S++)
        {
            ...
            // spin_new[dim_S*xyzi + j_S] = spin_local[j_S];
            // Energy_nu = Energy_nu + field_site[dim_S*xyzi + j_S] * spin_local[j_S];
            E_new += field_local[j_S] * spin_local[j_S];
        }
    ...
    int linear_Metropolis_sweep(int iter)
    {
        int site_i;
        do
        {
    ...
    int random_Metropolis_sweep(int iter)
    {
        int xyzi;
    ...
    int checkerboard_Metropolis_sweep(int iter)
    {
        ...
        while(iter > 0)
        {
            // #pragma acc parallel loop private(black_or_white) async, \
                present ( \
                            black_white_checkerboard, \
                            spin, \
                            h_random, \
            ...
            for (i=0; i < no_of_black_white_sites[black_or_white]; i++)
            {
                ...
                int site_index = black_white_checkerboard[black_or_white*no_of_black_white_sites[0] + i];
                ...
                double field_local[dim_S];
                double E_old = 0; // Energy_old(xyzi, spin_local, field_local);
                // #pragma acc loop reduction(+:E_old)
                for (j_S=0; j_S<dim_S; j_S++)
                {
                    field_local[j_S] = 0.0;
                    for (j_L=0; j_L<dim_L; j_L++)
                    {
                        for (k_L=0; k_L<2; k_L++)
                        {
                            field_local[j_S] = field_local[j_S] - (J[j_L] + J_random[2*dim_L*site_index + 2*j_L + k_L]) * spin[dim_S*N_N_I[2*dim_L*site_index + 2*j_L + k_L] + j_S];
                        }
                    }
                    field_local[j_S] = field_local[j_S] - (h[j_S] + h_random[dim_S*site_index + j_S]);
                    E_old += field_local[j_S] * spin[dim_S*site_index + j_S];
                }
                double E_new = 0; // Energy_new(site_index, spin_local, field_local);
                double s_mod = 0;
                double limit = 0.01 * dim_S;
                ...
                // #pragma acc loop reduction(+:E_new)
                for(j_S=0; j_S<dim_S; j_S++)
                {
                    spin_local[j_S] = spin_local[j_S] / s_mod;
                    E_new += field_local[j_S] * spin_local[j_S];
                ...
                // {
                    for (j_S=0; j_S<dim_S; j_S++)
                    {
                        spin[dim_S*site_index + j_S] = spin_local[j_S];
                    }
...
//====================      Glauber                          ====================//
    ...
    // #pragma acc routine seq
    double update_probability_Glauber(int xyzi)
    {
        double update_prob;
        double spin_local[dim_S];
        double field_local[dim_S];
    ...
    int linear_Glauber_sweep(int iter)
    {
        int site_i;
        do
        {
    ...
    int random_Glauber_sweep(int iter)
    {
        int xyzi;
        do
        {
    ...
    int checkerboard_Glauber_sweep(int iter)
    {
        // THIS PART IS PARALLELIZABLE. *modifications may be needed
        static int black_or_white = 0;
        int i;
        ...
        while(iter > 0)
        {
            // #pragma acc kernels \
                present ( \
                            black_white_checkerboard[0:2*no_of_black_white_sites[0]], \
                            spin[0:dim_S*no_of_sites], \
                            h_random[0:dim_S*no_of_sites], \
                            J_random[0:2*dim_L*no_of_sites], \
                            N_N_I[0:2*dim_L*no_of_sites], \
...
//====================      MonteCarlo-Wolff/Cluster         ====================//
    ...
    int generate_random_axis()
    {
        ...
        do
        {
            ...
            for(j_S=0; j_S<dim_S; j_S=j_S+1)
            {
                ...
                reflection_plane[j_S] = (-1.0 + 2.0 * some_rand_func_0_1);
    ...
    int transform_spin(int xyzi, double* __restrict__ spin_new)
    {
        double Si_dot_ref = 0;
        int j_S;
    ...
    double E_site_new(int xyzi, double* __restrict__ spin_new)
    {
        double energy_site = 0;
    ...
    double E_bond_old(int xyzi, int j_L, int k_L, int xyzi_nn)
    {
        double energy_site = 0;
        double Si_dot_ref = 0;
        int j_S;
    ...
    double E_bond_new(int xyzi, int j_L, int k_L, double* __restrict__ spin_new)
    {
        double energy_site = 0;
        double Si_dot_ref = 0;
        int j_S;
    ...
    int nucleate_from_site(int xyzi)
    {
        ...
        for (j_L=0; j_L<dim_L; j_L++)
        {
            for (k_L=0; k_L<2; k_L++)
            {
                int xyzi_nn = N_N_I[2*dim_L*xyzi + 2*j_L + k_L];
                if (cluster[xyzi_nn] == 0)
                {
                    ...
                    if (delta_E_bond < 0)
                    {
                        ...
                        double r_bond = some_rand_func_0_1;
                        /* if (r_bond < p_bond)
                        {
                        ...
                        if (p_bond > 0)
                        {
                            ...
                            // {
                                ...
                                double r_site = some_rand_func_0_1;
    ...
    int random_Wolff_sweep(int iter)
    {
        int xyzi;
        int i, j_S;
        for (i=0; i<iter; i++)
...
//====================      MonteCarlo-Sweep                 ====================//
    ...
    int Monte_Carlo_Sweep(int sweeps)
    {
        if (Gl_Me_Wo == 0)
        {
            if (Ch_Ra_Li == 0)
...
//====================      T!=0                             ====================//
    ...
    int thermalizing_iteration(int thermal_iter)
    {
        int j_S;
    ...
    int averaging_iteration(int average_iter)
    {
        ...
        while(average_iter)
        {
            Monte_Carlo_Sweep(1+(rand())%sampling_inter);
    ...
    int evolution_at_T(int repeat_for_same_T)
    {
        ...
        // create file name and pointer. 
        {
            ...
            pos += sprintf(pos, "}_%d_%d_%d_%d.dat", h_order, r_order, thermal_i, average_j);
            ...
            fprintf(pFile_1, "\t order_h=%d\t order_r=%d\t thermalizing-MCS=%d\t averaging-MCS=%d\t \n", h_order, r_order, thermal_i, average_j);
    ...
    int zfc_zfh_or_both(int c_h_b)
    {
        ...
        // create file name and pointer. 
        {
            ...
            fprintf(pFile_1, "\t order_h=%d\t order_r=%d\t (thermalizing-MCS,averaging-MCS)/{/Symbol D}T=(%d,%d)/%lf\t \n", h_order, r_order, thermal_i, average_j, delta_T);
        }
    ...
    int hysteresis_average(int hysteresis_iter)
    {
        int j_S;
        double MCS_counter = 0;
    ...
    int hysteresis_protocol(int jj_S)
    {
        ...
        printf("Hysteresis looping %d-times at T=%lf.. with MCS/field = %d \n", hysteresis_repeat, T, hysteresis_MCS);
        ...
        // create file name and pointer. 
        {
            ...
            pos += sprintf(pos, "}_%d_%d_%d.dat", h_order, r_order, hysteresis_MCS);
            pFile_1 = fopen(output_file_0, "a");
            ...
            fprintf(pFile_1, "\t order_h=%d\t order_r=%d\t MCS/{/Symbol d}h=%d/%lf\t \n", h_order, r_order, hysteresis_MCS, delta_h);
        }
...
//====================      RFIM ZTNE                        ====================//
    ...
    int find_extreme(double s, int remaining_sites)
    {
        h[0] = 0;
        int i_1 = 0;
    ...
    int flip_unstable(int nucleation_site, int remaining_sites)
    {
        int j_SS = 0, j_L, k_L;
        int next_site = nucleation_site;
    ...
    int zero_temp_RFIM_hysteresis()
    {
        ...
        int remaining_sites = no_of_sites;
        ensemble_m();
        ensemble_E();
        fprintf(pFile_1, "%lf\t ", h[jj_S]);
        for(j_S=0; j_S<dim_S; j_S++)
...
//====================      RFXY ZTNE                        ====================//
    ...
    // #pragma acc routine seq
    double Energy_minimum_old_XY(int xyzi, double* __restrict__ spin_local)
    {
        int j_S, j_L, k_L;
        double Energy_min=0.0;
        double field_local[dim_S];
    ...
    // #pragma acc routine seq
    double Energy_minimum_new_XY(int xyzi, double* __restrict__ spin_local)
    {
        int j_S, j_L, k_L;
        double Energy_min = 0.0;
        double field_local[dim_S];
    ...
    // #pragma acc routine seq
    double update_to_minimum_checkerboard(int xyzi, double* __restrict__ spin_local)
    {
        int j_S;
        ...
        for (h[jj_S] = h_start; order[jj_S] * h[jj_S] >= order[jj_S] * h_end; h[jj_S] = h[jj_S] - order[jj_S] * delta_h)
        {
            ...
            do 
            {
                    ...
                    copyin ( T, h[0:dim_S], J[0:dim_L] )
                {
                    ...
                    {
                        int site_index = black_white_checkerboard[black_or_white*no_of_black_white_sites[0] + site_i];
                        double spin_local[dim_S];
...
//====================      Main                             ====================//
    ...
    int free_memory()
    {
        ...
        #ifdef iscuda
        {
            cudaFree(spin_device);
            cudaFree(h_random_device);
            cudaFree(J_random_device);
            cudaFree(N_N_I_device);
            cudaFree(black_white_checkerboard_device);
            cudaFree(order_device);
            cudaFree(h_device);
            cudaFree(J_device);
            cudaFree(some_rand_func_0_1_device);
            cudaFree(some_rand_func_int_device);
            cudaFree(T_device);
            cudaFree(no_of_sites_device);
            cudaFree(no_of_black_white_sites_device);
        }
        #endif
    ...
    int for_cuda_parallelization()
    {
        ...
        cudaMalloc(&spin_device, dim_S*no_of_sites*sizeof(double));
        cudaMemcpy(spin_device, spin, dim_S*no_of_sites*sizeof(double), cudaMemcpyHostToDevice);

        cudaMalloc(&h_random_device, dim_S*no_of_sites*sizeof(double));
        cudaMemcpy(h_random_device, h_random, dim_S*no_of_sites*sizeof(double), cudaMemcpyHostToDevice);

        cudaMalloc(&J_random_device, 2*dim_L*no_of_sites*sizeof(double));
        cudaMemcpy(J_random_device, J_random, 2*dim_L*no_of_sites*sizeof(double), cudaMemcpyHostToDevice);

        cudaMalloc(&N_N_I_device, 2*dim_L*no_of_sites*sizeof(int));
        cudaMemcpy(N_N_I_device, N_N_I, 2*dim_L*no_of_sites*sizeof(int), cudaMemcpyHostToDevice);

        cudaMalloc(&black_white_checkerboard_device, 2*dim_L*no_of_sites*sizeof(int));
        cudaMemcpy(black_white_checkerboard_device, black_white_checkerboard, 2*dim_L*no_of_sites*sizeof(int), cudaMemcpyHostToDevice);

        cudaMalloc(&order_device, dim_S*sizeof(double));
        cudaMemcpy(order_device, order, dim_S*sizeof(double), cudaMemcpyHostToDevice);

        cudaMalloc(&h_device, dim_S*sizeof(double));
        cudaMemcpy(h_device, h, dim_S*sizeof(double), cudaMemcpyHostToDevice);

        cudaMalloc(&J_device, dim_L*sizeof(double));
        cudaMemcpy(J_device, J, dim_L*sizeof(double), cudaMemcpyHostToDevice);

        cudaMalloc(&some_rand_func_0_1_device, sizeof(double));
        cudaMemcpy(some_rand_func_0_1_device, &some_rand_func_0_1, sizeof(double), cudaMemcpyHostToDevice);

        cudaMalloc(&some_rand_func_int_device, sizeof(int));
        cudaMemcpy(some_rand_func_int_device, &some_rand_func_int, sizeof(int), cudaMemcpyHostToDevice);

        cudaMalloc(&T_device, sizeof(double));
        cudaMemcpy(T_device, &T, sizeof(double), cudaMemcpyHostToDevice);

        cudaMalloc(&no_of_sites_device, sizeof(int));
        cudaMemcpy(no_of_sites_device, &no_of_sites, sizeof(int), cudaMemcpyHostToDevice);

        cudaMalloc(&no_of_black_white_sites_device, 2*sizeof(int));
        cudaMemcpy(no_of_black_white_sites_device, no_of_black_white_sites, 2*sizeof(int), cudaMemcpyHostToDevice);
              
        printf("\n Memcpy done.\n");
        
        // cudaMemcpy( spin, spin_device, dim_S*no_of_sites * sizeof(int), cudaMemcpyDeviceToHost ) ;
    ...
    int aliasing_test()
    {
        ...
        printf("\n init done.\n");
        double ***spi_NN = (double ***)malloc(no_of_sites*dim_S*sizeof(double**)); 
        for(i=0; i<no_of_sites; i=i+1)
        {
            for(j_S=0; j_S<dim_S; j_S=j_S+1)
            {

                spi_NN[dim_S*i+j_S] = (double **)malloc(dim_L*2*sizeof(double*)); 
            }
        }
        printf("\n aliasing done.\n");
        ...
        for(i=0; i<no_of_sites; i=i+1)
        {
            for(j_S=0; j_S<dim_S; j_S=j_S+1)
            {
                for(j_L=0; j_L<dim_L; j_L=j_L+1)
                {
                    for(k_L=0; k_L<2; k_L=k_L+1)
                    {
                        spi_NN[dim_S*i+j_S][2*j_L+k_L] = &spin[dim_S*N_N_I[i*2*dim_L + 2*j_L + k_L]+j_S];
                    }
                }
            }        
        }
        for(i=0; i<no_of_sites; i=i+1)
        {
            for(j_S=0; j_S<dim_S; j_S=j_S+1)
            {
                printf("%lf ", *spi_NN[dim_S*i+j_S][3]);
            }        
            printf("\n");
        }
        ...
        for(i=0; i<no_of_sites; i=i+1)
        {
            for(j_S=0; j_S<dim_S; j_S=j_S+1)
            {
                printf("%lf ", *spi_NN[dim_S*i+j_S][1]);
            }        
            printf("\n");
        }
    ...
    int main()
    {
        ...
        load_J_config();
        printf("J loaded. \n");
        spin = (double *)calloc( dim_S*no_of_sites, sizeof(double) );
        // spin = (double*)malloc(dim_S*no_of_sites*sizeof(double));
        cluster = (int*)malloc(no_of_sites*sizeof(int));
        sorted_h_index = (int*)malloc(no_of_sites*sizeof(int));
        next_in_queue = (int*)malloc((1+no_of_sites)*sizeof(int));
        ...
        printf("hysteresis_MCS_multiplier = %d, hysteresis_MCS_max = %d\n", hysteresis_MCS_multiplier, hysteresis_MCS_max);

        printf("RAND_MAX = %lf,\n sizeof(int) = %ld,\n sizeof(long) = %ld,\n sizeof(double) = %ld,\n sizeof(int) = %ld,\n sizeof(short int) = %ld,\n sizeof(unsigned int) = %ld,\n sizeof(RAND_MAX) = %ld\n", (double)RAND_MAX, sizeof(int), sizeof(long), sizeof(double), sizeof(int), sizeof(short int), sizeof(unsigned int), sizeof(RAND_MAX));
