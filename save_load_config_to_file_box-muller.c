...
double generate_gaussian() // Marsaglia polar method
{
    ...
    do
    {
        rho = (double) rand () / RAND_MAX;
        theta = (double) rand () / RAND_MAX;
    }
    while (rho == 1 || theta == 1);
    
    mult = sqrt (-2 * log (1.0-rho));
    X1 = mult * sin(2*pie*theta);
    X2 = mult * cos(2*pie*theta);
...
int initialize_nearest_neighbor_index()
{
    ...
    NN = (long int*)malloc(2*dim_L*no_of_sites*sizeof(long int));    
    ...
    for(i=0; i<no_of_sites; i=i+1)
    {
        for(j_L=0; j_L<dim_L; j_L=j_L+1)
        {
            ...
            {
                NN[i*2*dim_L + 2*j_L + k_L] = nearest_neighbor(i, j_L, k_L);
            }
        }
    }
    return 0;
...
int initialize_J_random_gaussian()
{
    ...
    for(j_L=0; j_L<dim_L; j_L=j_L+1)
    {
        ...
        for(i=0; i<no_of_sites; i=i+1)
        {
            ...
            J_random[2*dim_L*NN[2*dim_L*i + 2*j_L] + 2*j_L + 1] = J_random[2*dim_L*i + 2*j_L];
