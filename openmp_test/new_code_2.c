        ...
        while(iter > 0)
        {
            for (j_S=0; j_S<dim_S; j_S++)
            {
                ...
                for (i=0; i < no_of_black_white_sites[black_or_white]; i++)
                {
                    ...
                    field_site[j_S*no_of_sites + site_index] = - (h[j_S] + h_random[j_S*no_of_sites + site_index]);
                }
            }
            for (j_S=0; j_S<dim_S; j_S++)
            {
                for (j_L=0; j_L<dim_L; j_L++)
                {
                    for (k_L=0; k_L<2; k_L++)
                    {
                        ...
                        {
                            long int site_index = black_white_checkerboard[black_or_white][i];
                            field_site[j_S*no_of_sites + site_index] += - (J[2*j_L + k_L] + J_random[(2*j_L + k_L)*no_of_sites + site_index]) * spin[j_S*no_of_sites + N_N_I[ (2*j_L + k_L)*no_of_sites + site_index] ];
                        }
                    }
                }
            }
            ...
            for (i=0; i < no_of_black_white_sites[black_or_white]; i++)
            {
                ...
                do
                {
                    ...
                    for(j_S=0; j_S<dim_S; j_S=j_S+1)
                    {
                        spin_temp[j_S*no_of_sites + site_index] = (1.0 - 2.0 * (double)rand_r(&random_seed[cache_size*omp_get_thread_num()])/(double)(RAND_MAX));
                        s_mod_site[site_index] = s_mod_site[site_index] + spin_temp[j_S*no_of_sites + site_index] * spin_temp[j_S*no_of_sites + site_index];
                    }
                }
            ...
            for(j_S=0; j_S<dim_S; j_S++)
            {
                ...
                {
                    long int site_index = black_white_checkerboard[black_or_white][i];
                    spin_temp[j_S*no_of_sites + site_index] = spin_temp[j_S*no_of_sites + site_index] / s_mod_site[site_index];
                    del_E_site[site_index] = del_E_site[site_index] + field_site[j_S*no_of_sites + site_index] * ( spin_temp[j_S*no_of_sites + site_index] - spin[j_S*no_of_sites + site_index] );
                }
            }
            ...
            for(j_S=0; j_S<dim_S; j_S++)
            {
                ...
                for (i=0; i < no_of_black_white_sites[black_or_white]; i++)
                {
                    ...
                    {
                        spin[j_S*no_of_sites + site_index] = spin_temp[j_S*no_of_sites + site_index];
                    }
                    else
                    {
                        spin[j_S*no_of_sites + site_index] = spin[j_S*no_of_sites + site_index];
                    }
                }
            }
