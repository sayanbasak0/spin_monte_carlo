...
//====================      on-site field (h)                ====================//
    ...
    double *del_E_site;
    double *s_mod_site;
    double *update_prob_site;
    double *random_variable_site;

//====================      Temperature                      ====================//
    double T = 3.00;
    double Temp_min = 0.10;
    double Temp_max = 1.00;
    double delta_T = 0.05;
...
//====================      MonteCarlo-tools                 ====================//
    ...
    int calculate_field(long int xyzi, double* restrict spin_local, double* restrict field_local)
    {
        int j_S, j_L, k_L;
        ...
        for (j_S=0; j_S<dim_S; j_S++)
        {
            ...
            for (j_L=0; j_L<dim_L; j_L++)
            {
                for (k_L=0; k_L<2; k_L++)
                {
                    // field_local[j_S] = field_local[j_S] - (J[2*j_L + k_L] + J_random[2*dim_L*xyzi + 2*j_L + k_L]) * (*spin_NN[2*j_L + k_L][dim_S*xyzi + j_S]);
                    // field_local[j_S] = field_local[j_S] - (J[2*j_L + k_L] + J_random[2*dim_L*xyzi + 2*j_L + k_L]) * (*spin_NN[dim_S*xyzi + j_S][2*j_L + k_L]);
                    field_local[j_S] = field_local[j_S] - (J[2*j_L + k_L] + J_random[2*dim_L*xyzi + 2*j_L + k_L]) * spin[dim_S*N_N_I[2*dim_L*xyzi + 2*j_L + k_L] + j_S];
                }
            }
            field_local[j_S] = field_local[j_S] - (h[j_S] + h_random[dim_S*xyzi + j_S]);
        }
    ...
    double Energy_change(long int xyzi, double* restrict spin_local, double* restrict field_local)
    {
        int j_S, j_L, k_L;
        double Energy_del=0.0, s_mod=0.0;
        double limit = 0.01 * dim_S;
        ...
        for(j_S=0; j_S<dim_S; j_S++)
        {
            ...
            field_local[j_S] = field_local[j_S] - (h[j_S] + h_random[dim_S*xyzi + j_S]);
            spin_local[j_S] = spin_local[j_S] / s_mod;
            Energy_del = Energy_del + field_local[j_S] *( spin_local[j_S] - spin[dim_S*xyzi + j_S] );
        }
...
//====================      Metropolis                       ====================//
    ...
    double update_probability_Metropolis(long int xyzi)
    {
        ...
        calculate_field(xyzi, spin_local, field_local);
        double E_old = Energy_old(xyzi, spin_local, field_local);
        double E_new = Energy_new(xyzi, spin_local, field_local);
        
        double E_del = E_new - E_old;
        // double E_del = Energy_change(xyzi, spin_local, field_local);
        if (E_del <= 0)
        {
            update_prob = 1.0;
        }
    ...
    int checkerboard_Metropolis_sweep(long int iter)
    {
        ...
        while(iter > 0)
        {
            ...
            for (i=0; i < no_of_black_white_sites[black_or_white]; i++)
            {
                ...
                for (j_S=0; j_S<dim_S; j_S++)
                {
                    // field_site[dim_S*site_index+j_S] = 0.0;
                    field_site[dim_S*site_index+j_S] = - (h[j_S] + h_random[dim_S*site_index + j_S]);
                    ...
                    for (j_L=0; j_L<dim_L; j_L++)
                    {
                        for (k_L=0; k_L<2; k_L++)
                        {
                            // field_local[j_S] = field_local[j_S] - (J[2*j_L + k_L] + J_random[2*dim_L*xyzi + 2*j_L + k_L]) * (*spin_NN[2*j_L + k_L][dim_S*xyzi + j_S]);
                            // field_local[j_S] = field_local[j_S] - (J[2*j_L + k_L] + J_random[2*dim_L*xyzi + 2*j_L + k_L]) * (*spin_NN[dim_S*xyzi + j_S][2*j_L + k_L]);
                            field_site[dim_S*site_index+j_S] += - (J[2*j_L + k_L] + J_random[2*dim_L*site_index + 2*j_L + k_L]) * spin[dim_S*N_N_I[2*dim_L*site_index + 2*j_L + k_L] + j_S];
                        }
                    }
                    // field_site[dim_S*site_index+j_S] = field_site[dim_S*site_index+j_S] - (h[j_S] + h_random[dim_S*site_index + j_S]);
                }

                do
                {
                    s_mod_site[site_index]=0.0;
                    for(j_S=0; j_S<dim_S; j_S=j_S+1)
                    {
                        spin_temp[dim_S*site_index+j_S] = (1.0 - 2.0 * (double)rand_r(&random_seed[cache_size*omp_get_thread_num()])/(double)(RAND_MAX));
                        s_mod_site[site_index] = s_mod_site[site_index] + spin_temp[dim_S*site_index+j_S] * spin_temp[dim_S*site_index+j_S];
                    }
                }
                while(s_mod_site[site_index] >= 1 || s_mod_site[site_index] <= 0.02);
                s_mod_site[site_index] = sqrt(s_mod_site[site_index]);
                
                del_E_site[site_index] = 0.0;
                for(j_S=0; j_S<dim_S; j_S++)
                {
                    spin_temp[dim_S*site_index+j_S] = spin_temp[dim_S*site_index+j_S] / s_mod_site[site_index];
                    del_E_site[site_index] = del_E_site[site_index] + field_site[dim_S*site_index+j_S] * ( spin_temp[dim_S*site_index+j_S] - spin[dim_S*site_index + j_S] );
                }

                if (del_E_site[site_index] <= 0)
                {
                    update_prob_site[site_index] = 1.0;
                }
                else
                {
                    update_prob_site[site_index] = exp(-(del_E_site[site_index])/T);                    
                }
                random_variable_site[site_index] = (double) rand_r(&random_seed[cache_size*omp_get_thread_num()])/ (double) RAND_MAX;
                if (random_variable_site[site_index] < update_prob_site[site_index])
                {
                    for(j_S=0; j_S<dim_S; j_S++)
                    {
                        spin[dim_S*site_index+j_S] = spin_temp[dim_S*site_index+j_S];
                    }
                }
                else
                {
                    for(j_S=0; j_S<dim_S; j_S++)
                    {
                        spin[dim_S*site_index+j_S] = spin[dim_S*site_index+j_S];
                    }
                }
            }
...
//====================      Main                             ====================//
    ...
    int free_memory()
    {
        ...
        // free(del_E_site) ;
        // free(s_mod_site) ;
        // free(update_prob_site) ;
        // free(random_variable_site) ;
    ...
    int alias_nearest_neighbor()
    {
        
        int i, j_S, j_L, k_L;
        
        // spin_NN = (double ***)malloc(2*dim_L*sizeof(double**)); 
        // for(j_L=0; j_L<dim_L; j_L=j_L+1)
        // {
        //     for(k_L=0; k_L<2; k_L=k_L+1)
        //     {
        //         spin_NN[2*j_L+k_L] = (double **)malloc(dim_S*no_of_sites*sizeof(double*)); 
        //     }
        // }
        // for(i=0; i<no_of_sites; i=i+1)
        // {
        ...
        //             for(k_L=0; k_L<2; k_L=k_L+1)
        //             {
        //                 spin_NN[2*j_L+k_L][dim_S*i+j_S] = &spin[dim_S*N_N_I[i*2*dim_L + 2*j_L + k_L]+j_S];
        //             }
        //         }
        //     }        
        // }

        spin_NN = (double ***)malloc(no_of_sites*dim_S*sizeof(double**)); 
        for(i=0; i<no_of_sites; i=i+1)
        {
            for(j_S=0; j_S<dim_S; j_S=j_S+1)
            {
                spin_NN[dim_S*i+j_S] = (double **)malloc(dim_L*2*sizeof(double*)); 
            }
        }
        for(i=0; i<no_of_sites; i=i+1)
        {
            for(j_S=0; j_S<dim_S; j_S=j_S+1)
            {
                for(j_L=0; j_L<dim_L; j_L=j_L+1)
                {
                    for(k_L=0; k_L<2; k_L=k_L+1)
                    {
                        spin_NN[dim_S*i+j_S][2*j_L+k_L] = &spin[dim_S*N_N_I[i*2*dim_L + 2*j_L + k_L]+j_S];
                    }
                }
            }        
        }
        ...
        for(i=0; i<no_of_sites; i=i+1)
        {
            for(j_S=0; j_S<dim_S; j_S=j_S+1)
            {
                printf("%lf ", *spin_NN[dim_S*i+j_S][3]);
            }        
            printf("\n");
        } */
    ...
    int main()
    {
        ...
        del_E_site = (double*)malloc(no_of_sites*sizeof(double));
        s_mod_site = (double*)malloc(no_of_sites*sizeof(double));
        update_prob_site = (double*)malloc(no_of_sites*sizeof(double));
        random_variable_site = (double*)malloc(no_of_sites*sizeof(double));
        

        long int i, j;
        // alias_nearest_neighbor();
