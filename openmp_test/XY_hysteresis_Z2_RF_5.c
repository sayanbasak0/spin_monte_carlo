...
//====================      MC-update iterations             ====================//
    long int thermal_i = 1*10*10*8*10; // ! *=lattice_size
    long int average_j = 1*10*10*8; // ! *=lattice_size
        ...
        for (j_S=0; j_S<dim_S; j_S++)
        {
            ...
            for (j_L=0; j_L<dim_L; j_L++)
            {
                for (k_L=0; k_L<2; k_L++)
                {
                    ...
                    field_local[j_S] += - (J[2*j_L + k_L] + J_random[2*dim_L*xyzi + 2*j_L + k_L]) * (*spin_NN[dim_S*xyzi + j_S][2*j_L + k_L]);
                    // field_local[j_S] = field_local[j_S] - (J[2*j_L + k_L] + J_random[2*dim_L*xyzi + 2*j_L + k_L]) * spin[dim_S*N_N_I[2*dim_L*xyzi + 2*j_L + k_L] + j_S];
                }
            }
            field_local[j_S] += - (h[j_S] + h_random[dim_S*xyzi + j_S]);
        }
