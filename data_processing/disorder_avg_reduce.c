...
int main(int argc, char** argv)
{
    ...
    for (ci=0; ci<config_vals[input] ; ci++)
    {
        ...
        pos_in_1 += sprintf(pos_in_1, "transient_O2_2D_%d_reduced.dat", L_vals[Li] );
        ...
        pos_in_2 += sprintf(pos_in_2, "limit_cycle_O2_2D_%d_reduced.dat", L_vals[Li] );
    ...
    for (ci=0; ci<config_vals[input] ; ci++)
    {
        for (hi=0; hi<len_h_field_vals; hi++)
        {
            ...
            for (i_col=2; i_col<no_of_columns_in-3; i_col++)
            {
                ...
                {
                    array_tr_out[hi][i_col+9] += array_tr[ci][hi][i_col] / array_tr[ci][hi][1];
                }
                array_lc_out[hi][i_col+9] += array_lc[ci][hi][i_col] / array_lc[ci][hi][1];
                array_to_out[hi][i_col+9] += (array_tr[ci][hi][i_col] + array_lc[ci][hi][i_col]) / (array_tr[ci][hi][1] + array_lc[ci][hi][1]);
            }
            for (i_col=no_of_columns_in-3; i_col<no_of_columns_in; i_col++)
            {
                if ( array_tr[ci][hi][i_col] > array_tr_out[hi][i_col+9] )
                {
                    array_tr_out[hi][i_col+9] = array_tr[ci][hi][i_col] ;
                }
                if ( array_lc[ci][hi][i_col] > array_lc_out[hi][i_col+9] )
                {
                    array_lc_out[hi][i_col+9] = array_lc[ci][hi][i_col] ;
                }
                if ( array_tr[ci][hi][i_col] > array_lc[ci][hi][i_col] )
                {
                    if ( array_tr[ci][hi][i_col] > array_to_out[hi][i_col+9] )
                    {
                        array_to_out[hi][i_col+9] = (array_tr[ci][hi][i_col] ) ;
                    }
                }
                else
                {
                    if ( array_lc[ci][hi][i_col] > array_to_out[hi][i_col+9] )
                    {
                        array_to_out[hi][i_col+9] = (array_lc[ci][hi][i_col] ) ;
                    }
    ...
    pos_out_1 += sprintf(pos_out_1, "transient_O2_2D_%d_dis_avg.dat", L_vals[Li] );

    pFile_output_1 = fopen(output_file_1, "a");
    
    char output_file_2[256];
    char *pos_out_2 = output_file_2;
    pos_out_2 += sprintf(pos_out_2, "limit_cycle_O2_2D_%d_dis_avg.dat", L_vals[Li] );

    pFile_output_2 = fopen(output_file_2, "a");
    
    char output_file_3[256];
    char *pos_out_3 = output_file_3;
    pos_out_3 += sprintf(pos_out_3, "total_O2_2D_%d_dis_avg.dat", L_vals[Li] );
    ...
    fprintf(pFile_output_1, "---<(dmx/dphi)^2>---\t" );
    fprintf(pFile_output_1, "---<(dmy/dphi)^2>---\t" );
    fprintf(pFile_output_1, "----<(dm/dphi)^2>---\t" );
    ...
    fprintf(pFile_output_1, "<(dmx/dphi)^2/dphi>-\t" );
    fprintf(pFile_output_1, "<(dmy/dphi)^2/dphi>-\t" );
    fprintf(pFile_output_1, "-<(dm/dphi)^2/dphi>-\t" );
    ...
    fprintf(pFile_output_2, "---<(dmx/dphi)^2>---\t" );
    fprintf(pFile_output_2, "---<(dmy/dphi)^2>---\t" );
    fprintf(pFile_output_2, "----<(dm/dphi)^2>---\t" );
    ...
    fprintf(pFile_output_2, "<(dmx/dphi)^2/dphi>-\t" );
    fprintf(pFile_output_2, "<(dmy/dphi)^2/dphi>-\t" );
    fprintf(pFile_output_2, "-<(dm/dphi)^2/dphi>-\t" );
    ...
    fprintf(pFile_output_3, "---<(dmx/dphi)^2>---\t" );
    fprintf(pFile_output_3, "---<(dmy/dphi)^2>---\t" );
    fprintf(pFile_output_3, "----<(dm/dphi)^2>---\t" );
    ...
    fprintf(pFile_output_3, "<(dmx/dphi)^2/dphi>-\t" );
    fprintf(pFile_output_3, "<(dmy/dphi)^2/dphi>-\t" );
    fprintf(pFile_output_3, "-<(dm/dphi)^2/dphi>-\t" );
