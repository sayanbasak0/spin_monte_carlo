...
int main(int argc, char** argv)
{
    ...
    for (ci=0; ci<config_vals[input] ; ci++)
    {
        ...
        for (hi=0; hi<len_h_field_vals; hi++)
        {
            ...
            double dmx_sq[2] = { 0.00, 0.00 };
            double dmy_sq[2] = { 0.00, 0.00 };
            double dm_sq[2] = { 0.00, 0.00 };
            double dmx_max[2] = { 0.00, 0.00 };
            double dmy_max[2] = { 0.00, 0.00 };
            double dm_max[2] = { 0.00, 0.00 };
            double dmx_sq_dphi_sq[2] = { 0.00, 0.00 };
            double dmy_sq_dphi_sq[2] = { 0.00, 0.00 };
            double dm_sq_dphi_sq[2] = { 0.00, 0.00 };

            #pragma omp parallel for private(i_loop,j_array) reduction(+:dmx_sq[:2], dmy_sq[:2], dm_sq[:2], dmx_sq_dphi_sq[:2], dmy_sq_dphi_sq[:2], dm_sq_dphi_sq[:2]) reduction( max:dmx_max[:2], dmy_max[:2], dm_max[:2] )
            for (i_loop=0; i_loop<=loop_no; i_loop++)
            {
                ...
                for (j_array=0; j_array<nth_loop_array_size[i_loop]; j_array++)
                {
                    ...
                    dmx_sq[i_loop] += del_mx_sq;
                    dmy_sq[i_loop] += del_my_sq;
                    dm_sq[i_loop] += del_m_sq;

                    if (del_phi[i_loop][j_array] > 0)
                    {
                        double del_phi_sq = del_phi[i_loop][j_array] * del_phi[i_loop][j_array];
                        dmx_sq_dphi_sq[i_loop] += del_mx_sq / del_phi_sq;
                        dmy_sq_dphi_sq[i_loop] += del_my_sq / del_phi_sq;
                        dm_sq_dphi_sq[i_loop] += del_m_sq / del_phi_sq;
                    }
                }
            ...
            pos_out_1 += sprintf(pos_out_1, "transient_O2_2D_%d_reduced.dat", L_vals[Li] );

            pFile_output_1 = fopen(output_file_1, "a");
            
            fprintf(pFile_output_1, "%.14e\t", h_field_vals[hi] );
            fprintf(pFile_output_1, "%.14e\t", dphi[0] );
            fprintf(pFile_output_1, "%.14e\t", dmx_sq[0] );
            fprintf(pFile_output_1, "%.14e\t", dmy_sq[0] );
            fprintf(pFile_output_1, "%.14e\t", dm_sq[0] );
            fprintf(pFile_output_1, "%.14e\t", dmx_sq_dphi_sq[0] );
            fprintf(pFile_output_1, "%.14e\t", dmy_sq_dphi_sq[0] );
            fprintf(pFile_output_1, "%.14e\t", dm_sq_dphi_sq[0] );
            ...
            pos_out_2 += sprintf(pos_out_2, "limit_cycle_O2_2D_%d_reduced.dat", L_vals[Li] );

            pFile_output_2 = fopen(output_file_2, "a");
            
            fprintf(pFile_output_2, "%.14e\t", h_field_vals[hi] );
            fprintf(pFile_output_2, "%.14e\t", dphi[1] );
            fprintf(pFile_output_2, "%.14e\t", dmx_sq[1] );
            fprintf(pFile_output_2, "%.14e\t", dmy_sq[1] );
            fprintf(pFile_output_2, "%.14e\t", dm_sq[1] );
            fprintf(pFile_output_2, "%.14e\t", dmx_sq_dphi_sq[1] );
            fprintf(pFile_output_2, "%.14e\t", dmy_sq_dphi_sq[1] );
            fprintf(pFile_output_2, "%.14e\t", dm_sq_dphi_sq[1] );
        ...
        pFile_output_1 = fopen("header_reduced_data.dat", "a");
        
        fprintf(pFile_output_1, "-------column label------" );
        fprintf(pFile_output_1, "\n" );
        fprintf(pFile_output_1, "H\t" );
        fprintf(pFile_output_1, "dphi\t" );
        fprintf(pFile_output_1, "dmx_sq\t" );
        fprintf(pFile_output_1, "dmy_sq\t" );
        fprintf(pFile_output_1, "dm_sq\t" );
        fprintf(pFile_output_1, "dmx_sq_dphi_sq\t" );
        fprintf(pFile_output_1, "dmy_sq_dphi_sq\t" );
        fprintf(pFile_output_1, "dm_sq_dphi_sq\t" );
