...
int main(int argc, char** argv)
{
    ...
    double h_field_vals[] = { 0.032, 0.034, 0.036, 0.038, 0.040, 0.042, 0.044, 0.046, 0.048, 0.050, 0.052, 0.054, 0.056, 0.058 };
    ...
    for (ci=0; ci<config_vals[input] ; ci++)
    {
        ...
        for (hi=0; hi<len_h_field_vals; hi++)
        {
            ...
            #pragma omp parallel for private(i_loop,j_array) reduction(+:dphi[:2], dmx_sq_dphi[:2], dmy_sq_dphi[:2], dm_sq_dphi[:2], dmx_abs[:2], dmy_abs[:2], dm_abs[:2], dmx_abs_avg[:2], dmy_abs_avg[:2], dm_abs_avg[:2], dmx_sq_avg[:2], dmy_sq_avg[:2], dm_sq_avg[:2], number_of_avalanche[:2]) reduction(max:dmx_max[:2], dmy_max[:2], dm_max[:2] )
            ...
            if (dphi[1] < 0.9 )
            {
                printf("\n%d\t%ld\t%lf\t%d\t%le\t%le\t%.17e\n", 1, nth_loop_array_size[1], h_field_vals[hi], ci, phi[1][nth_loop_array_size[1]-1], phi[1][0], dphi[1]);
                return 0;
            }
