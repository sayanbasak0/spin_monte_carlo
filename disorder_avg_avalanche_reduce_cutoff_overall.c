...
int main(int argc, char** argv)
{
    ...
    for (ci=0; ci<config_vals[input] ; ci++)
    {
        ...
        pos_in_1 += sprintf(pos_in_1, "transient_O2_2D_%d_avalanche_cutoff_overall.dat", L_vals[Li] );
        ...
        pos_in_2 += sprintf(pos_in_2, "limit_cycle_O2_2D_%d_avalanche_cutoff_overall.dat", L_vals[Li] );
    ...
    for (ci=0; ci<config_vals[input] ; ci++)
    {
        for (hi=0; hi<len_h_field_vals; hi++)
        {
            ...
            if (array_tr[ci][hi][8] > 0)
            {
                array_tr_out[hi][0] += 1.0/array_tr[ci][hi][8];
            }
            
            if (array_lc[ci][hi][8] > 0)
            {
                array_lc_out[hi][0] += 1.0/array_lc[ci][hi][8];
            }
            
            if ( (array_tr[ci][hi][8] + array_lc[ci][hi][8]) > 0)
            {
                array_to_out[hi][0] += 1.0/(array_tr[ci][hi][8] + array_lc[ci][hi][8]);
            }
            ...
            for (i_col=2; i_col<no_of_columns_in; i_col++)
            {
                if (array_tr[ci][hi][8] > 0)
                {
                    array_tr_out[hi][i_col+6] += array_tr[ci][hi][i_col] / array_tr[ci][hi][8];
                    array_tr_out_stdev[hi][i_col+6] += (array_tr[ci][hi][i_col] / array_tr[ci][hi][8]) * (array_tr[ci][hi][i_col] / array_tr[ci][hi][8]);
                }
                
                if (array_lc[ci][hi][8] > 0)
                {
                    array_lc_out[hi][i_col+6] += (array_lc[ci][hi][i_col] / array_lc[ci][hi][8]);
                    array_lc_out_stdev[hi][i_col+6] += (array_lc[ci][hi][i_col] / array_lc[ci][hi][8]) * (array_lc[ci][hi][i_col] / array_lc[ci][hi][8]);
                }

                if ( (array_tr[ci][hi][8] + array_lc[ci][hi][8]) > 0)
                {
                    array_to_out[hi][i_col+6] += (array_tr[ci][hi][i_col] + array_lc[ci][hi][i_col]) / (array_tr[ci][hi][8] + array_lc[ci][hi][8]);
                    array_to_out_stdev[hi][i_col+6] += ((array_tr[ci][hi][i_col] + array_lc[ci][hi][i_col]) / (array_tr[ci][hi][8] + array_lc[ci][hi][8])) * ((array_tr[ci][hi][i_col] + array_lc[ci][hi][i_col]) / (array_tr[ci][hi][8] + array_lc[ci][hi][8]));
                }
            ...
            for (i_col=2; i_col<no_of_columns_in; i_col++)
            {
                array_tr_out[hi][i_col+12] += array_tr[ci][hi][i_col] * array_tr[ci][hi][8];
                array_tr_out_stdev[hi][i_col+12] += (array_tr[ci][hi][i_col] * array_tr[ci][hi][8]) * (array_tr[ci][hi][i_col] * array_tr[ci][hi][8]);
                
                array_lc_out[hi][i_col+12] += (array_lc[ci][hi][i_col] * array_lc[ci][hi][8]);
                array_lc_out_stdev[hi][i_col+12] += (array_lc[ci][hi][i_col] * array_lc[ci][hi][8]) * (array_lc[ci][hi][i_col] * array_lc[ci][hi][8]);

                array_to_out[hi][i_col+12] += (array_tr[ci][hi][i_col] + array_lc[ci][hi][i_col]) * (array_tr[ci][hi][8] + array_lc[ci][hi][8]);
                array_to_out_stdev[hi][i_col+12] += ((array_tr[ci][hi][i_col] + array_lc[ci][hi][i_col]) * (array_tr[ci][hi][8] + array_lc[ci][hi][8])) * ((array_tr[ci][hi][i_col] + array_lc[ci][hi][i_col]) * (array_tr[ci][hi][8] + array_lc[ci][hi][8]));
            }
        }
    }
    ...
    pos_out_1 += sprintf(pos_out_1, "transient_O2_2D_%d_dis_avalanche_cutoff_overall.dat", L_vals[Li] );

    pFile_output_1 = fopen(output_file_1, "a");
    
    char output_file_2[256];
    char *pos_out_2 = output_file_2;
    pos_out_2 += sprintf(pos_out_2, "limit_cycle_O2_2D_%d_dis_avalanche_cutoff_overall.dat", L_vals[Li] );

    pFile_output_2 = fopen(output_file_2, "a");
    
    char output_file_3[256];
    char *pos_out_3 = output_file_3;
    pos_out_3 += sprintf(pos_out_3, "total_O2_2D_%d_dis_avalanche_cutoff_overall.dat", L_vals[Li] );
    ...
    pFile_output_3 = fopen(output_file_3, "a");
    {
        ...
        fprintf(pFile_output_1, "--<<dmx^2>/delphi>--\t" );
        fprintf(pFile_output_1, "-s<<dmx^2>/delphi>--\t" );
        fprintf(pFile_output_1, "--<<dmy^2>/delphi>--\t" );
        fprintf(pFile_output_1, "-s<<dmy^2>/delphi>--\t" );
        fprintf(pFile_output_1, "--<<dm^2>/delphi>---\t" );
        fprintf(pFile_output_1, "-s<<dm^2>/delphi>---\t" );
        fprintf(pFile_output_1, "--<<|dmx|>/delphi>--\t" );
        fprintf(pFile_output_1, "-s<<|dmx|>/delphi>--\t" );
        fprintf(pFile_output_1, "--<<|dmy|>/delphi>--\t" );
        fprintf(pFile_output_1, "-s<<|dmy|>/delphi>--\t" );
        fprintf(pFile_output_1, "--<<|dm|>/delphi>---\t" );
        fprintf(pFile_output_1, "-s<<|dm|>/delphi>---\t" );
        ...
        fprintf(pFile_output_2, "--<<dmx^2>/delphi>--\t" );
        fprintf(pFile_output_2, "-s<<dmx^2>/delphi>--\t" );
        fprintf(pFile_output_2, "--<<dmy^2>/delphi>--\t" );
        fprintf(pFile_output_2, "-s<<dmy^2>/delphi>--\t" );
        fprintf(pFile_output_2, "--<<dm^2>/delphi>---\t" );
        fprintf(pFile_output_2, "-s<<dm^2>/delphi>---\t" );
        fprintf(pFile_output_2, "--<<|dmx|>/delphi>--\t" );
        fprintf(pFile_output_2, "-s<<|dmx|>/delphi>--\t" );
        fprintf(pFile_output_2, "--<<|dmy|>/delphi>--\t" );
        fprintf(pFile_output_2, "-s<<|dmy|>/delphi>--\t" );
        fprintf(pFile_output_2, "--<<|dm|>/delphi>---\t" );
        fprintf(pFile_output_2, "-s<<|dm|>/delphi>---\t" );
        ...
        fprintf(pFile_output_3, "--<<dmx^2>/delphi>--\t" );
        fprintf(pFile_output_3, "-s<<dmx^2>/delphi>--\t" );
        fprintf(pFile_output_3, "--<<dmy^2>/delphi>--\t" );
        fprintf(pFile_output_3, "-s<<dmy^2>/delphi>--\t" );
        fprintf(pFile_output_3, "--<<dm^2>/delphi>---\t" );
        fprintf(pFile_output_3, "-s<<dm^2>/delphi>---\t" );
        fprintf(pFile_output_3, "--<<|dmx|>/delphi>--\t" );
        fprintf(pFile_output_3, "-s<<|dmx|>/delphi>--\t" );
        fprintf(pFile_output_3, "--<<|dmy|>/delphi>--\t" );
        fprintf(pFile_output_3, "-s<<|dmy|>/delphi>--\t" );
        fprintf(pFile_output_3, "--<<|dm|>/delphi>---\t" );
        fprintf(pFile_output_3, "-s<<|dm|>/delphi>---\t" );
    ...
    for (hi=0; hi<len_h_field_vals; hi++)
    {
        ...
        for (i_col=2; i_col<no_of_columns_in; i_col++)
        {
            if ( array_tr_out[hi][0] > 0 )
            {
                fprintf(pFile_output_1, "%.14e\t", array_tr_out[hi][i_col+6] / array_tr_out[hi][0] );
                fprintf(pFile_output_1, "%.14e\t", sqrt( fabs( (array_tr_out_stdev[hi][i_col+6] / array_tr_out[hi][0]) - (array_tr_out[hi][i_col+6] / array_tr_out[hi][0])*(array_tr_out[hi][i_col+6] / array_tr_out[hi][0]) ) ) );
            }
            else
            {
                fprintf(pFile_output_1, "%.14e\t", 0.0 );
                fprintf(pFile_output_1, "%.14e\t", 0.0 );
            }
            
            if ( array_lc_out[hi][0] > 0 )
            {
                fprintf(pFile_output_2, "%.14e\t", array_lc_out[hi][i_col+6] / array_lc_out[hi][0] );
                fprintf(pFile_output_2, "%.14e\t", sqrt( fabs( (array_lc_out_stdev[hi][i_col+6] / array_lc_out[hi][0]) - (array_lc_out[hi][i_col+6] / array_lc_out[hi][0])*(array_lc_out[hi][i_col+6] / array_lc_out[hi][0]) ) ) );
            }
            else
            {
                fprintf(pFile_output_2, "%.14e\t", 0.0 );
                fprintf(pFile_output_2, "%.14e\t", 0.0 );
            }


            if ( array_to_out[hi][0] > 0 )
            {
                fprintf(pFile_output_3, "%.14e\t", array_to_out[hi][i_col+6] / array_to_out[hi][0] );
                fprintf(pFile_output_3, "%.14e\t", sqrt( fabs( (array_to_out_stdev[hi][i_col+6] / array_to_out[hi][0]) - (array_to_out[hi][i_col+6] / array_to_out[hi][0])*(array_to_out[hi][i_col+6] / array_to_out[hi][0]) ) ) );
            }
            else
            {
                fprintf(pFile_output_3, "%.14e\t", 0.0 );
                fprintf(pFile_output_3, "%.14e\t", 0.0 );
            }
        }
        for (i_col=2; i_col<no_of_columns_in; i_col++)
        {
            if ( array_tr_out[hi][8] > 0 )
            {
                fprintf(pFile_output_1, "%.14e\t", array_tr_out[hi][i_col+12] / array_tr_out[hi][8] );
                fprintf(pFile_output_1, "%.14e\t", sqrt( fabs( (array_tr_out_stdev[hi][i_col+12] / array_tr_out[hi][8]) - (array_tr_out[hi][i_col+12] / array_tr_out[hi][8])*(array_tr_out[hi][i_col+12] / array_tr_out[hi][8]) ) ) );
            }
            else
            {
                fprintf(pFile_output_1, "%.14e\t", 0.0 );
            ...
            if ( array_lc_out[hi][8] > 0 )
            {
                fprintf(pFile_output_2, "%.14e\t", array_lc_out[hi][i_col+12] / array_lc_out[hi][8] );
                fprintf(pFile_output_2, "%.14e\t", sqrt( fabs( (array_lc_out_stdev[hi][i_col+12] / array_lc_out[hi][8]) - (array_lc_out[hi][i_col+12] / array_lc_out[hi][8])*(array_lc_out[hi][i_col+12] / array_lc_out[hi][8]) ) ) );
            }
            else
            {
                fprintf(pFile_output_2, "%.14e\t", 0.0 );
            ...
            if ( array_to_out[hi][8] > 0 )
            {
                fprintf(pFile_output_3, "%.14e\t", array_to_out[hi][i_col+12] / array_to_out[hi][8] );
                fprintf(pFile_output_3, "%.14e\t", sqrt( fabs( (array_to_out_stdev[hi][i_col+12] / array_to_out[hi][8]) - (array_to_out[hi][i_col+12] / array_to_out[hi][8])*(array_to_out[hi][i_col+12] / array_to_out[hi][8]) ) ) );
            }
            else
            {
                fprintf(pFile_output_3, "%.14e\t", 0.0 );
